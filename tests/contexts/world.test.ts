...
describe("WorldContext", () =>
{
    ...
    describe("Subscribe", () =>
    {
        it("Should subscribe to an event and receive it when emitted", () =>
        {
        ...
        it("Should allow subscribing multiple handlers to the same event", () =>
        {
    ...
    describe("Wait", () =>
    {
        it("Should wait for an event and resolve with its arguments", async () =>
        {
            const _moveHandler = vi.fn();
        ...
        it("Should timeout if the event isn't emitted in time", async () =>
        {
            _populateWorld(new System());
    ...
    {
        it("Should unsubscribe a specific handler from an event", () =>
        {
            const _moveHandler1 = vi.fn();
            const _moveHandler2 = vi.fn();
        ...
        it("Should throw when unsubscribing from an event that doesn't exist", () =>
        {
            const _moveHandler = vi.fn();
            const _spawnHandler = vi.fn();
        ...
        it("Should throw when unsubscribing a handler that wasn't subscribed", () =>
        {
            const _moveHandler = vi.fn();
    ...
    describe("Dispose", () =>
    {
        it("Should clear all subscribers when the system is removed", () =>
        {
            const _spawnHandler1 = vi.fn();
            const _spawnHandler2 = vi.fn();
            const _moveHandler1 = vi.fn();
        ...
        it("Should clear all subscribers when the context is disposed", () =>
        {
            const _spawnHandler1 = vi.fn();
            const _spawnHandler2 = vi.fn();
            const _moveHandler1 = vi.fn();
